# Analysis — Curaçao vs Ivory Coast

_as_of 2026-06-25 · source: snapshot (snapshot 2026-06-24T16:00:07-06:00)_

## 1X2 (match winner)

- Model: **Curaçao 13%** · Draw 22% · **Ivory Coast 65%**

| sel | model | sharp fair | best | verdict |
|---|---|---|---|---|
| home | 13% | 5% | +2300 | **pass** |
| draw | 22% | 11% | +850 | **pass** |
| away | 65% | 84% | -526 | **bet** |

## Goals (total)

- Expected goals: Curaçao 0.62 – 1.82 Ivory Coast  (total λ 2.44)

| over | model P | odds | EV@odds |
|---|---|---|---|
| 1.5 | 70% | — | — |
| 2.5 | 44% | -192 | -0.33 |
| 3.5 | 23% | — | — |

## Otros mercados

_EV@odds = P(modelo)×odds−1, al precio ofrecido (no de-vig). En mercados eficientes (1X2, doble oport.) un EV+ suele ser error nuestro (edge test); en goles es la señal viva._

| mercado | model P | odds | EV@odds |
|---|---|---|---|
| Doble oport. Curaçao | 35% | — | — |
| Doble oport. Ivory Coast | 87% | — | — |
| Over 1.5 goles Curaçao | 13% | — | — |
| Over 1.5 goles Ivory Coast | 54% | — | — |
| BTTS (ambos marcan) | 39% | — | — |
| Tiros Curaçao over 9.5 | 26% | — | — |
| Tiros Ivory Coast over 9.5 | 98% | — | — |
| TaP Curaçao over 2.5 | 46% | — | — |
| TaP Ivory Coast over 2.5 | 95% | — | — |

## Player shots-on-target (CLOV-on-overs)

_No prop candidates — skipped (snapshot backtest — props weren't captured)._

## Recommendation

- **1x2 away** @ -526 — soft price beats the sharp fair by +3.5% (prospective CLOV+).

_Caveats: 1X2/goals edge requires beating a soft book, not out-predicting the sharp (edge test). Props are one-sided — un-de-viggable, graded by CLOV. Snapshot backtests use pre-kickoff odds, which may be stale if captured long before kickoff._

## Resultado y checks (qué se cumplió)

- **Curaçao 0–2 Ivory Coast** (away, 2 goles)

| Mercado | model P | ¿Pasó? | check |
|---|---|---|---|
| Gana Curaçao | 13% | no | ✅ |
| Empate | 22% | no | ✅ |
| Gana Ivory Coast | 65% | sí | ✅ |
| Doble oport. Curaçao | 35% | no | ✅ |
| Doble oport. Ivory Coast | 87% | sí | ✅ |
| Over 1.5 goles | 70% | sí | ✅ |
| Over 2.5 goles | 44% | no | ✅ |
| Over 3.5 goles | 23% | no | ✅ |
| Over 1.5 goles Curaçao | 13% | no | ✅ |
| Over 1.5 goles Ivory Coast | 54% | sí | ✅ |
| BTTS | 39% | no | ✅ |

**Checks acertados: 11/11** (✅ = la inclinación del modelo coincidió con lo que pasó).
