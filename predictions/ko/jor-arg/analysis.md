# Analysis — Jordan vs Argentina

_as_of 2026-06-28 · source: live_

## 1X2 (match winner)

- Model: **Jordan 2%** · Draw 10% · **Argentina 87%**

| sel | model | sharp fair | best | verdict |
|---|---|---|---|---|
| home | 2% | 5% | +2400 | **pass** |
| draw | 10% | 10% | +900 | **pass** |
| away | 87% | 85% | -588 | **pass** |

## Goals (total)

- Expected goals: Jordan 0.37 – 2.82 Argentina  (total λ 3.19)

| over | model P | odds | EV@odds |
|---|---|---|---|
| 1.5 | 83% | -556 | -0.02 |
| 2.5 | 62% | -172 | -0.02 |
| 3.5 | 40% | +148 | -0.02 |

## Otros mercados

_EV@odds = P(modelo)×odds−1, al precio ofrecido (no de-vig). En mercados eficientes (1X2, doble oport.) un EV+ suele ser error nuestro (edge test); en goles es la señal viva._

| mercado | model P | odds | EV@odds |
|---|---|---|---|
| Doble oport. Jordan | 13% | +600 | -0.12 |
| Doble oport. Argentina | 98% | -2500 | +0.02 |
| Over 0.5 goles Jordan | 31% | +160 | -0.20 |
| Over 2.5 goles Argentina | 54% | -130 | -0.05 |
| BTTS (ambos marcan) | 29% | +182 | -0.18 |
| Tiros Jordan over 9.5 | 6% | — | — |
| Tiros Argentina over 9.5 | 100% | — | — |
| TaP Jordan over 2.5 | 26% | — | — |
| TaP Argentina over 2.5 | 100% | — | — |

## Props de delanteros (tiros y tiros a puerta)

Todos los delanteros (FW) de ambos equipos, con las líneas que el libro cotiza para **tiros totales** y **tiros a puerta**. `model` = P(over) del modelo; EV = model×odds−1 al precio (over-only, sin de-vig). ✅ = el modelo le gana al precio con vig y es ≥50% (candidato, se juzga por CLOV).

| equipo | delantero | mercado | línea | model | odds | EV | valor |
|---|---|---|---|---|---|---|---|
| Argentina | Julian Alvarez | tiros | o0.5 | 93% | -2000 | -0.03 |  |
| Argentina | Julian Alvarez | tiros | o1.5 | 74% | -357 | -0.05 |  |
| Argentina | Julian Alvarez | tiros | o2.5 | 49% | -125 | -0.12 |  |
| Argentina | Julian Alvarez | tiros | o3.5 | 27% | +180 | -0.24 |  |
| Argentina | Julian Alvarez | tiros | o4.5 | 13% | +350 | -0.42 |  |
| Argentina | Julian Alvarez | tiros | o5.5 | 5% | +700 | -0.58 |  |
| Argentina | Julian Alvarez | a puerta | o0.5 | 60% | -357 | -0.23 |  |
| Argentina | Julian Alvarez | a puerta | o1.5 | 23% | +128 | -0.47 |  |
| Argentina | Julian Alvarez | a puerta | o2.5 | 6% | +350 | -0.71 |  |
| Argentina | Julian Alvarez | a puerta | o3.5 | 1% | +850 | -0.86 |  |
| Argentina | Lautaro Martinez | tiros | o0.5 | 97% | -2500 | +0.00 | ✅ |
| Argentina | Lautaro Martinez | tiros | o1.5 | 85% | -417 | +0.06 | ✅ |
| Argentina | Lautaro Martinez | tiros | o2.5 | 66% | -141 | +0.12 | ✅ |
| Argentina | Lautaro Martinez | tiros | o3.5 | 44% | +155 | +0.11 |  |
| Argentina | Lautaro Martinez | tiros | o4.5 | 25% | +300 | +0.01 |  |
| Argentina | Lautaro Martinez | tiros | o5.5 | 13% | +600 | -0.12 |  |
| Argentina | Lautaro Martinez | a puerta | o0.5 | 69% | -357 | -0.12 |  |
| Argentina | Lautaro Martinez | a puerta | o1.5 | 33% | +120 | -0.28 |  |
| Argentina | Lautaro Martinez | a puerta | o2.5 | 11% | +330 | -0.51 |  |
| Argentina | Lautaro Martinez | a puerta | o3.5 | 3% | +900 | -0.69 |  |

🎯 **Value (CLOV-on-overs)** — solo paper/CLOV, apostar chico: Lautaro Martinez tiros o0.5 (EV +0.00), Lautaro Martinez tiros o1.5 (EV +0.06), Lautaro Martinez tiros o2.5 (EV +0.12).

_Cotizados pero fuera de la tabla (no FW o sin datos de tiros): Abdullah Nasib (sin datos de tiros), Ali Al-Azaizeh (pos MF), Ali Olwan (sin datos de tiros), Exequiel Palacios (pos MF), Giovani Lo Celso (pos MF), Giuliano Simeone (pos DF), Husam Abu Dahab (sin datos de tiros), Ihsan Haddad (sin datos de tiros), Leandro Paredes (pos MF), Marcos Senesi (pos DF), Mohannad Abu Taha (sin datos de tiros), Nicolas Otamendi (pos DF), Nicolas Tagliafico (pos DF), Nizar Al-Rashdan (sin datos de tiros), Noor Al Rawabdeh (sin datos de tiros), Odeh Fakhoury (sin datos de tiros), Yazan Al Arab (sin datos de tiros)._

## Recommendation

- **No 1X2/goals bet** — nothing where a soft book beats the sharp fair (the common, correct outcome). Big model-vs-sharp gaps are *suspect*, not value.

_Caveats: 1X2/goals edge requires beating a soft book, not out-predicting the sharp (edge test). Props are one-sided — un-de-viggable, graded by CLOV. Snapshot backtests use pre-kickoff odds, which may be stale if captured long before kickoff._

